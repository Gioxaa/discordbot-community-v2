...
module.exports = {
  execute: async (interaction) => {
    ...
    // Target validation: Ambil member yang dituju dari opsi command
    ...
    // Database check: Pastikan custom role milik target ada di database
    ...
    // Role execution: Ambil role custom dari guild
    try {
      ...
      // Deletion process: Hapus role custom dari guild
      await role.delete(`Deleted by ${interaction.user.tag} via /deletecostumroles`);
      
      // Update custom role database: Hapus entri custom role dari database
      database.roles = database.roles.filter(entry => entry.userId !== member.id);
      writeDatabase(database);

      // Update gift database: Hapus entri gift untuk booster jika ada
      const giftDB = require('../utils/giftDatabase').readGiftDatabase();
      if (giftDB && Array.isArray(giftDB.gifts)) {
        giftDB.gifts = giftDB.gifts.filter(entry => entry.boosterId !== member.id);
        require('../utils/giftDatabase').writeGiftDatabase(giftDB);
      }
